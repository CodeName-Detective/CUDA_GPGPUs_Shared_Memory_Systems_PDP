...
// Kernel to perform matrix multiplication of M1 and M2^T (transpose) with tiled corner turning 
// M1 and M2 are assumed to be stored in row-major layout.

__global__ void matmul_kernel_tiled_corner_turning(float* M1, float* M2, float* M_Out,  unsigned int left_dim, unsigned int inner_dim, unsigned int right_dim){
    ...
    for(int phase=0; phase<(inner_dim+TILE_WIDTH-1)/TILE_WIDTH; ++phase){
        ...
        //Load M2s as Transpose into share memory - Handling Boundary Conditions
        if((phase*TILE_WIDTH+threadIdx.y)<inner_dim && col<right_dim){
            M2s[threadIdx.x][threadIdx.y] = M2[((phase*TILE_WIDTH+threadIdx.y)*right_dim)+(col)];
        }
        else{
            M2s[threadIdx.x][threadIdx.y] = 0.0f;
        }
